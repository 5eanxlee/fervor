...
const age = (value?: string, observedAt?: string) => {
    if (!value) return '—';
    const reference = observedAt ? new Date(observedAt).getTime() : Date.now();
    const seconds = Math.max(0, Math.floor((reference - new Date(value).getTime()) / 1000));
...
export default function DiscoveryTerminal() {
    ...
    const [tokens, setTokens] = useState<DiscoveryToken[]>(pumpSnapshot);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                if (!(settings.pauseOnHover && hover.current)) setTokens(next.length ? next : pumpSnapshot);
                setLive(response.success && next.length > 0);
            } catch {
...
function TokenCard({ token, settings }: {
    ...
    const trades = token.buyCount5m + token.sellCount5m;
    const buyRate = trades ? Math.round(token.buyCount5m / trades * 100) : undefined;
    const edge = buyRate === undefined
        ? 'var(--term-border-strong)'
        : buyRate >= 50 ? 'var(--trench-good-edge)' : 'var(--trench-bad-edge)';
                ...
                <div className="w-[clamp(4.9rem,5.75vw,5.25rem)] shrink-0">
                    <div className="relative h-[clamp(4.9rem,9.2vh,5.25rem)] w-full">
                        ...
                        {settings.visionProgress === 'ring' ? <span
                            ...
                            style={{ borderColor: edge, color: buyRate === undefined ? 'var(--term-muted)' : buyRate >= 50 ? 'var(--trench-positive)' : 'var(--trench-negative)' }}
                        >◒</span> : <span className="pointer-events-none absolute inset-x-0 bottom-0 z-10 h-1 overflow-hidden rounded-b-md bg-[var(--term-border-strong)]"><i className="block h-full bg-[var(--term-accent)]" style={{ width: `${buyRate ?? 0}%` }} /></span>}
                ...
                <div className="flex h-full min-w-0 flex-1 flex-col">
                    <div className="flex min-w-0 items-start gap-1.5 pr-[10.5rem]">
                        ...
                        <div className="absolute right-[clamp(.75rem,.8vw,.875rem)] top-[.5625rem] shrink-0 text-right text-[.75rem] leading-none tabular-nums">
                            ...
                            <div className="mt-[.1875rem] flex items-center justify-end gap-1 text-[.6875rem]"><span className="text-[var(--term-dim)]">L</span><span className="font-[500] text-[var(--term-text)]">{money(token.liquidityUsd, settings.visionCaps === 'rounded')}</span></div>
                        </div>
                    </div>
                    ...
                    <div className="token-meta mt-0 flex h-[1.125rem] min-w-max items-center gap-[.5rem] whitespace-nowrap text-[.75rem] text-[var(--term-muted)]">
                        ...
                        {settings.showStats && <div className="token-meta-stats flex min-w-0 items-center gap-[.5rem]">
                            ...
                            <span className="flex items-center gap-1"><ChartBarIcon className="h-4 w-4" />{compact(trades, 0)}</span>
                            <span className="flex items-center gap-1"><span className="text-[var(--trench-positive)]">B</span>{compact(token.buyCount5m, 0)}<span className="text-[var(--trench-negative)]">S</span>{compact(token.sellCount5m, 0)}</span>
                            <span className="flex items-center gap-1"><EyeIcon className="h-4 w-4" />{compact(token.replyCount, 0)}</span>
                        </div>}
                    </div>
                    ...
                    {token.creator && <div className="token-creator mt-0 flex h-4 min-w-max items-center gap-1.5 whitespace-nowrap text-[.75rem] leading-none">
                        {token.creatorHandle && token.socials?.twitter
                            ? <a href={token.socials.twitter} target="_blank" rel="noreferrer" className="truncate text-[var(--term-cyan)] hover:text-white">@{token.creatorHandle}</a>
                            : token.creatorHandle && <span className="truncate text-[var(--term-cyan)]">@{token.creatorHandle}</span>}
                        <span className="text-[var(--term-cyan)]">{short(token.creator)}</span>
                    </div>}

                    <div className="token-chips mt-[.1875rem] flex min-w-0 items-center gap-[.25rem] overflow-hidden whitespace-nowrap text-[.75rem]">
                        <Chip icon={<UserIcon />} value={buyRate === undefined ? '— buys' : `${buyRate}% buys`} tone={buyRate === undefined ? 'neutral' : buyRate > 55 ? 'good' : 'bad'} />
                        <Chip icon={<ClockIcon />} value={`${compact(trades, 0)} tx`} />
                        <Chip icon={<CircleStackIcon />} value={`${compact(token.buyCount5m, 0)} B`} tone="good" />
                        <Chip icon={<FireIcon />} value={`${compact(token.sellCount5m, 0)} S`} tone={token.sellCount5m ? 'bad' : 'neutral'} />
                        <button
