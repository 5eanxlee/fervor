...
`replay-lab.ts` exposes that owner as newline-delimited JSON over stdin and stdout, not an HTTP listener. It imports no application environment loader and starts paused after verifying the corpus and a small, read-only paper-model JSON file. Launch it from a clean environment with `npm run replay:lab --workspace backend -- --replay <dir> --checkpoints <dir> --model <file> --run <id>`. Commands are `status`, `play`, `pause`, `step`, `seek`, `place`, `cancel`, bounded `orders` and `facts` pages, `checkpoint`, and `stop`. A seek deliberately starts an empty paper timeline in a newer epoch, so orders and fills from the future cannot leak backward. Graceful EOF or a termination signal pauses and writes one composite session checkpoint.
